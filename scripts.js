...
function searchResults() {
  ...
  if (student) {
    document.getElementById(
      "student-name"
    ).innerText = `Student Name: ${student.name}`;
    document.getElementById(
      "student-id"
    ).innerText = `Register Number: ${student.slNo}`;

    const studentClass = studentID.startsWith("2025") ? "I PUC" : "II PUC";
    document.getElementById(
      "student-class"
    ).innerText = `Class: ${studentClass}`;
    ...
    const subjects = [
      ...
      { id: "english", max: 80, marks: toNumber(student.english_theory) },
      {
        ...
        max: 70,
        marks:
          toNumber(student.physics_theory) +
          toNumber(student.physics_practical),
      },
      {
        id: "chemistry",
        max: 70,
        marks:
          toNumber(student.chemistry_theory) +
          toNumber(student.chemistry_practical),
      },
      {
        id: "mathematics",
        max: 80,
        marks: toNumber(student.mathematics_theory),
      },
      {
        id: "biology",
        max: 70,
        marks:
          toNumber(student.biology_theory) +
          toNumber(student.biology_practical),
      },
    ];
    ...
    document.getElementById("marks-in-words").innerText =
    ...
    var finalResult = percentage >= 35 ? "PASS" : "FAIL";

    document.getElementById("final-result").innerText = finalResult;

    var resultElement = document.querySelector(".final-results");
    var resultTextElement = document.getElementById("final-result");

    // Display the result
    resultTextElement.textContent = finalResult;

    // Apply corresponding class for color coding
    if (finalResult === "PASS") {
